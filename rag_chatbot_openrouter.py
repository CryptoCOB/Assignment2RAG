"""
ReqMind — Hybrid RAG (LangChain + FAISS + OpenRouter)
...
- OpenRouter cloud models with lightweight routing (fast inference)
"""
import os
import sys
...
class SimpleRetrievalQA:
    ...
    def __init__(self, llm: ChatOpenAI, retriever, k: int = 3, max_context_chars: int = 8000):
        self.llm = llm
    ...
    def invoke(self, inputs):
        ...
        # ChatOpenAI returns an AIMessage or string depending on version
...
def choose_llm(query: str) -> ChatOpenAI:
    q = query.lower()
    tech = any(k in q for k in ["code", "function", "method", "class", "implementation", "algorithm"])
    vision = any(k in q for k in ["diagram", "figure", "image", "table", "chart"])
...
def build_chain(llm: ChatOpenAI, vectorstore: FAISS, k: int | None = None) -> SimpleRetrievalQA:
    if k is None:
...
def _summarize_chunks(llm: ChatOpenAI, texts: list[str], title: str = "Summary") -> str:
